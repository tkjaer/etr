...
The dashboards in `grafana/dashboards/` are generated by
[`grafana/gen_dashboards.py`](grafana/gen_dashboards.py) (Python 3, standard
library only). To change a dashboard, edit the script and regenerate:

```bash
python3 grafana/gen_dashboards.py   # rewrites grafana/dashboards/*.json
```

Grafana reloads provisioned dashboards within 10 seconds. You can still try
changes in the Grafana UI (`allowUiUpdates` is on), but port them back to the
script, or the next regeneration overwrites them.
