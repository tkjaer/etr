...
func Test_isARPEntryMatch_Linux(t *testing.T) {
	...
	for _, tt := range tests {
		entry := &rtnl.Neigh{IP: tt.entryIP, HwAddr: net.HardwareAddr{0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff}}
...
func Test_checkARPTable_Linux(t *testing.T) {
	...
	tests := []struct {
		...
		{"found", net.ParseIP("192.0.2.100").To4(), []*rtnl.Neigh{{IP: net.ParseIP("192.0.2.100"), HwAddr: mac1}}, nil, mac1, false},
		{"not found", net.ParseIP("192.0.2.100").To4(), []*rtnl.Neigh{{IP: net.ParseIP("192.0.2.200"), HwAddr: mac1}}, nil, nil, true},
		{"empty table", net.ParseIP("192.0.2.100").To4(), []*rtnl.Neigh{}, nil, nil, true},
		{"table error", net.ParseIP("192.0.2.100").To4(), nil, errors.New("rtnetlink error"), nil, true},
		{"multiple entries", net.ParseIP("198.51.100.5").To4(), []*rtnl.Neigh{
			{IP: net.ParseIP("198.51.100.1"), HwAddr: mac1},
			{IP: net.ParseIP("198.51.100.5"), HwAddr: mac2},
		}, nil, mac2, false},
		{"ipv6", net.ParseIP("fe80::1"), []*rtnl.Neigh{{IP: net.ParseIP("fe80::1"), HwAddr: mac1}}, nil, mac1, false},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			orig := getARPTable
			getARPTable = func(iface *net.Interface) ([]*rtnl.Neigh, error) { return tt.neighs, tt.err }
